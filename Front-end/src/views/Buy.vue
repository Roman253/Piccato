...
<div class="calendarDate">
 ...
 <v-date-picker mode='range' v-model='selectedDate' :min-date='new Date()' show-caps>
</v-date-picker>

  </div>

      <h3>{{ artwork.title }}</h3>
      <p class="artist">Artist: {{ artwork.artist }} <br></p>
      <p class="description">
      <img class="media-object" v-bind:src="artwork.artworkUrl" width="300px"> <br>    
          Description: {{artwork.description}}</p>
      <article class="price">Price per day: {{ artwork.price }} sek</article> <br>
      <article class="price">Your total price: {{ artwork.price * amount }} sek</article> <br>
...
export default {
  ...
  methods: {
    buy(){
      this.$store.dispatch('buy', { artwork: this.artwork._id, amount: this.amount });
